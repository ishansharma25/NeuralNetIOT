...
def predict_attack_type(model_path, result):
    try:
        ...
        # If predictions are 1D, handle them without using np.argmax
        if len(predictions.shape) == 1:
            predicted_class_index = predictions[0]
            confidence = 1.0  # If it's a single class output, confidence is always 1
        else:
        ...
        return {
            ...
            'full_probabilities': predictions[0].tolist() if len(predictions.shape) > 1 else predictions.tolist()
        }
